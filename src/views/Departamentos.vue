    ...
    <table class="table">
      ...
      <tbody>
        <tr v-for="(departamento, index) in departamentos" :key="departamento.depa_codi">
          ...
          <td>
            <button @click="deleteDepartamento(departamento.depa_codi)"
                    class="btn btn-danger mx-2">
              <font-awesome-icon icon="trash" />
            </button>
            <button @click="editDepartamento(departamento.depa_codi)"
...
export default {
  ...
  methods: {
    deleteDepartamento(codigo) {
      Swal.fire({
        title: `¿Desea eliminar el Departamento con id ${codigo}?`,
